...
class MyHomePage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    print('in Dashboard ${store.state}');
    return Scaffold(
      appBar: AppBar(
        title: Text(store.state.user == null ? 'Loading' : store.state.user.name),
      ),
...
class AddUserButton extends StatelessWidget {
  ...
  AddUserButton(this.model);

  @override
  Widget build(BuildContext context) {
    return RaisedButton(
      child: Text('AddUser'),
      onPressed: () => model.onAddUser(),
    );
  }
}
  ...
  Widget build(BuildContext context) {
    return ListView(
      children: model.items
        ...
        trailing: Checkbox(value: item.completed, onChanged: (b) { model.onCompleted(item);}),
      ))
          .toList(),
    );
  }
...
class _ViewModel {
  ...
  _ViewModel(
      {@required this.items,
        ...
        @required this.onRemoveItems, @required this.onAddUser});
  ...
  factory _ViewModel.create(Store<AppState> store) {
    ...
    _onAddUser() {
      store.dispatch(AddUserAction(User(name: faker.person.firstName(), id: 1)));
    }
